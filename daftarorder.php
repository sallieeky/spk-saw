...
//jika tidak ada session pengguna atau member di dalam sistem daftar order, maka lakukan login terlebih dahulu.
if(!isset($_SESSION['log'])){
	header('location:login.php');
} 

//jika ada session pengguna atau member di dalam sistem daftar order, maka tampil halaman daftar order..
else {
...
//jika pengguna ada perubahan pesanan produk di daftar order yang berdasarkan kode order.	
if(isset($_POST["update"])){
	$kode = $_POST['idproduknya'];
	$jumlah = $_POST['jumlah'];
	$q1 = mysqli_query($conn, "update detailorder set qty='$jumlah' where idproduk='$kode' and orderid='$orderidd'");
	
	//jika proses perubahan dalam pemesanan produk awal status tanpa kendala, maka beralih ke halaman Cart.
	if($q1){

		echo "Berhasil Update Cart
		<meta http-equiv='refresh' content='1; url= cart.php'/>";

	//jika ada kesalahan pada saat proses perubahan dalam pemesanan produk awal status tanpa kendala, maka beralih ke halaman Cart.
	} else {
		echo "Gagal update cart
		<meta http-equiv='refresh' content='1; url= cart.php'/>";
	} 

//jika pengguna ada menghapus data pesanan produk awal yang berdasarkan kode order. 	
...
//jika ada pengguna melakukan pembatalan pesanan produk.
...
//jika ada pengguna melakukan penyelesaian pesanan produk.
		...
		<div class="container">
			...
			<div class="checkout-right">
				<table class="timetable_sub">
					...
					//jika ada data pesanan produk yang belum masuk ke keranjang (cart), maka hitung total seluruh belanja satu per satu.
					if ($itungtrans3 > 0) {
					...
					<tr class="rem1"><form method="post">
						...
						<td class="invert">
							<?php
							//jika status checkout pada pemesanan produk tersebut masih "Waiting", maka bisa klik untuk konfirmasi pembayaran. 
							if($b['status'] == 'Waiting' ){ 
							...
							<?php
							 ...
							 //jika status checkout pada pemesanan produk tersebut masih "Pengiriman", maka bisa klik untuk konfirmasi penerimaan pesan produk.
					...
					<?php } ?>
					<?php } 
					//jika daftar belanja produk tidak ada atau tidak ditemukan.
					else { ?>
					<tr>
