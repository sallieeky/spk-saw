	...
	<div class="products">
		<div class="container">
			...
			<div class="col-md-8 products-right">
				...
				//Mencari data produk. 
				$key = '';
				//jika pengguna memasukkan kata kunci (key), maka menampilkan data nama produk yang berdasarkan kata kunci (key).
				if (isset($_GET['key'])) {
					$key = 'where namaproduk like "%'.$_GET['key'].'%"';
				}
				//jika pengguna memilih "all", maka menampilkan seluruh data nama produk.
				if ($idk == 'all') {
					$brgs = mysqli_query($conn,"SELECT * from produk ".$key." order by idproduk ASC");
				}
				//jika selain kedua diatas, maka menampilkan data nama produk yang berdasarkan ID Produk.
				else
				{
					...
					//Jika data nama produk ditemukan di database.
					if($x>0){
					...
					//Jika data nama produk tidak ditemukan di database.
					else {
