...
//jika ada menambah pesanan produk ke dalam keranjang (cart).
if(isset($_POST['addprod'])){
	//jika tidak ada aktivitas pelanggan di halaman ini, maka harus login terlebih dahulu.
	if(!isset($_SESSION['log']))
		{	
			header('location:login.php');
		} 
		//jika ada aktivitas pelanggan di halaman ini, maka terjadi penambahan produk ke dalam keranjang.
		else {
				$ui 	= $_SESSION['id'];
				...
				if($liat>0){
							...
							$tambahdata = mysqli_query($conn,"INSERT into detailorder 
																...
																values
																('$orid','$idproduk','1')");
							//jika tidak ada kesalahan atau error, maka penambahan produk telah berhasil.
							if ($tambahdata){	
							echo ("<script>
								    window.alert('Berhasil menambahkan barang ke keranjang');
								    window.location.href='product.php?idproduk=".$idproduk."';
							...
							//jika ada kesalahan atau error, maka penambahan produk telah gagal.
							else { 
